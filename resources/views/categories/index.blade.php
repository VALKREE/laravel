        ...
        <div class="container">
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                @foreach($categoriesList as $categories)
                    <div class="main_categories" id="{{ $categories['id'] }}" style="display: flex; flex-direction: column;">
                        <a href="{{ route('categories.show', ['id' => $categories['id']]) }}">{{ $categories['category'] }}</a>
                        <p class="main_categories"><b>Категория:</b>{{ $categories['category'] }}</p>
                    </div>
                @endforeach
            </div>
        </div>
